...
def test_송금_보냈_평어가_피해자_용어로_확장된다():
    ...
    assert "피해구제 신청" in _expand_query("finance", "모르는 사람한테 송금해버렸어요")  # '송금'
    assert "피해구제 신청" in _expand_query("finance", "계좌로 돈을 보냈어요")            # '보냈'


def test_보이스피싱은_방향무관_전기통신금융사기_우산용어를_항상_붙인다():
    # 통신사기피해환급법 본문 적중을 돕는 중립 용어 — 세 방향 모두에 필요.
    for q in (Q1, Q4, Q5):
        assert "전기통신금융사기" in _expand_query("finance", q)
